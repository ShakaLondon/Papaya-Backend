...
const userBusinessValidationRules = () => {
  return [
    ...
    body("username")
      ...
      .custom(async (value) => {
        return await BusinessUserModel.findOne({ username: value }).then(
          (user) => {
            if (user) {
    ...
    body("email")
      ...
      .custom(async (value) => {
        return await BusinessUserModel.findOne({ email: value }).then(
          (user) => {
            if (user) {
              return Promise.reject("Email is already in use, please Login!");
            }
    ...
    body("password")
      ...
      .withMessage(
        "Password must be greater than 8 and contain at least one uppercase letter, one lowercase letter, two numbers and one symbol!"
      ),
    body("website")
      .exists()
      .withMessage("Website is a mandatory field!")
      .isURL()
...
const userValidationRules = () => {
  ...
  return [
    ...
    body("dateOfBirth")
      .exists()
      .withMessage("DOB value is a mandatory field")
      .isDate()
      .withMessage("Date invalid")
      .isBefore(cA)
      .withMessage("You need to be older than 18!"),
  ];
};
...
const reviewValidationRules = () => {
  return [
    ...
    body("rating")
      .exists()
      .withMessage("Rating is a mandatory field!")
      .isInt({ max: 5, min: 0 })
    ...
    body("comment")
      ...
      .withMessage("Comment must be at least 50 charaters."),
    body("title")
      .exists()
    ...
    body("userID")
      .exists()
      .withMessage("UserID is a mandatory field!")
      .custom(async (value) => {
        return await ReviewModel.findOne({ userID: value }).then((user) => {
          // FIX THIS MUST SEARCH BUSINESS REVIEWS SO ONE REVIEW PER USER PER BUSINESS
          if (user) {
            return Promise.reject(
              "You can only leave one review per business."
            );
          }
        });
      }),
...
const businessValidationRules = () => {
  return [
    ...
    body("website")
      ...
      .withMessage("Must be a valid website")
      .custom(async (value) => {
        return await BusinessModel.findOne({ website: value }).then(
          (business) => {
            if (business) {
              return Promise.reject(
                "This business already has a Papaya page. Please use the search bar to find them."
              );
            }
          }
        );
...
const validations = {
  userBusinessValidationRules: userBusinessValidationRules,
  userValidationRules: userValidationRules,
  reviewValidationRules: reviewValidationRules,
  businessValidationRules: businessValidationRules,
  validate: validate,
};
