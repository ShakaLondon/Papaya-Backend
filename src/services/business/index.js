...
businessRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const updateCategory = await new CategoryModel.findByIdAndUpdate(
        catFind._id,
        {
          categoryList: newBusiness._id,
...
// GET ALL BUSINESSES ✅
businessRouter.get("/", JwtMiddleware, adminOnly, async (req, res, next) => {
  try {
  ...
  async (req, res, next) => {
    try {
      ...
      next(
        createError(
          500,
          `An error occurred while deleting user ${req.params.businessID}`
        )
      );
    }
  }
  ...
  async (req, res, next) => {
    try {
      ...
      next(
        createError(
          500,
          `An error occurred while updating user ${req.params.businessID}`
        )
      );
    }
  }
...
businessRouter.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const businessReviews = await ReviewModel.find({
