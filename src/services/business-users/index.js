      ...
      if (business) {
        if (business.businessUserID) {
          next(
            createError(
              404,
              `This business has already been claimed. If this is an error please contact us.`
            )
          );
        } else {
          const businessUser = await new BusinessUserModel({
            ...req.body,
            ...business,
          }).save();
          const updateBusiness = await BusinessModel.findByIdAndUpdate(
            business._id,
          ...
          const updateUser = await BusinessUserModel.findByIdAndUpdate(
            businessUser._id,
...
businessUserRouter.put(
  ...
  async (req, res, next) => {
    try {
      ...
      const updatedUser = await BusinessUserModel.findByIdAndUpdate(
        businessID,
        req.body,
        {
          new: true, // to use existing record n
