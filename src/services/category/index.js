...
async (req, res, next) => {
  try {
    ...
    const allCategories = await CategoryModel.find({ categoryLevel: 1 })
    .populate([{
      path: 'categoryItems',
      model: 'Business',
    },{
...
categoryRouter.get("/:categoryName",
// JwtMiddleware,
// adminOnly, 
async (req, res, next) => {
  try {
...
// GET SINGLE REVIEW
categoryRouter.get("/parent/:categoryID",
JwtMiddleware,
adminOnly, 
async (req, res, next) => {
  try {
...
async (req, res, next) => {
  ...
  const deletedCategory = await CategoryModel.findByIdAndDelete(reviewID)
    ...
    next(createError(500, `An error occurred while deleting category ${req.params.categoryID}`))
  }
})
...
async (req, res, next) => {
  try {
    ...
    next(createError(500, `An error occurred while updating category ${req.params.categoryID}`))
  }
})
